...
if grep -r "unchecked_add\|unchecked_sub\|unchecked_mul" program/src/ > /dev/null 2>&1; then
...
if grep -r "checked_add\|checked_sub\|checked_mul" program/src/ > /dev/null 2>&1; then
    echo "✅ Found proper overflow protection patterns"
else
    echo "⚠️  No overflow protection patterns found (may be in other modules)"
fi

# Test 3: Check for proper error handling
echo "Test 3: Checking for arithmetic error handling"
if grep -r "ArithmeticError\|overflow\|underflow" program/src/ > /dev/null 2>&1; then
    echo "✅ Found arithmetic error handling"
else
    echo "⚠️  No arithmetic error handling found (may be in other modules)"
fi

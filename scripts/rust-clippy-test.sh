...
if [ -f "program/clippy.toml" ] || [ -f "program/.clippy.toml" ] || grep -r "clippy" program/Cargo.toml > /dev/null 2>&1; then
    echo "✅ Found clippy configuration"
else
    echo "⚠️  No explicit clippy configuration found"
fi

# Test 2: Check for Rust code quality patterns
echo "Test 2: Checking for Rust code quality patterns"
if grep -r "#\[deny.*clippy\]\|#\[warn.*clippy\]" program/src/ > /dev/null 2>&1; then
    echo "✅ Found clippy lint directives"
else
    echo "⚠️  No explicit clippy lint directives found"
fi

# Test 3: Check for common Rust best practices
echo "Test 3: Checking for common Rust best practices"
if grep -r "unwrap()\|expect(" program/src/ > /dev/null 2>&1; then
    echo "⚠️  Found potential unwrap() usage (should use proper error handling)"
else
...
if grep -r "edition.*=.*2021" program/Cargo.toml > /dev/null 2>&1; then
    echo "✅ Found Rust 2021 edition"
else
    echo "⚠️  No Rust 2021 edition found"
fi

# Test 8: Check for dependency management
echo "Test 8: Checking for dependency management"
if [ -f "program/Cargo.toml" ] && grep -r "\[dependencies\]" program/Cargo.toml > /dev/null 2>&1; then
...
if grep -r "ark-crypto-primitives\|sha2\|ed25519" program/Cargo.toml > /dev/null 2>&1; then
    echo "✅ Found security-focused dependencies"
else
    echo "⚠️  Limited security-focused dependencies found"
fi

# Test 10: Check for proper use of Rust features
echo "Test 10: Checking for proper use of Rust features"
if grep -r "#\[derive\]\|#\[allow\]\|#\[deny\]" program/src/ > /dev/null 2>&1; then
    echo "✅ Found proper use of Rust features"
else
    echo "⚠️  Limited use of Rust features found"
fi

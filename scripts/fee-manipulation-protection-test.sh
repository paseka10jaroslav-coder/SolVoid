...
if grep -r "calculate_protocol_fee\|calculate_protocol_fee_atomic" program/src/ > /dev/null 2>&1; then
...
if grep -r "MAX_FEE_BASIS_POINTS\|FeeExceedsMaximum\|fee.*maximum" program/src/ > /dev/null 2>&1; then
...
if grep -r "emergency_fee_multiplier\|MAX_EMERGENCY_MULTIPLIER\|InvalidMultiplier" program/src/ > /dev/null 2>&1; then
...
if grep -r "FEE_CHANGE_DELAY_SECONDS\|multiplier_activation_time\|schedule_emergency_multiplier_change" program/src/ > /dev/null 2>&1; then
...
if grep -r "calculate_protocol_fee_atomic\|atomic.*fee\|checked_mul.*fee" program/src/ > /dev/null 2>&1; then
...
if grep -r "MIN_DEPOSIT_AMOUNT\|MAX_DEPOSIT_AMOUNT\|DepositTooSmall\|DepositTooLarge" program/src/ > /dev/null 2>&1; then
...
if grep -r "PROTOCOL_FEE_BASIS_POINTS\|basis.*points" program/src/ > /dev/null 2>&1; then
...
if grep -r "is_fee_emergency\|trigger_emergency_mode\|disable_emergency_mode" program/src/ > /dev/null 2>&1; then
...
if grep -r "final_fee.*>\|fee.*validation\|ensure.*fee" program/src/ > /dev/null 2>&1; then
...
if grep -r "PROTOCOL_FEE_BASIS_POINTS.*=.*[0-9]" program/src/ > /dev/null 2>&1; then
...
if grep -r "MAX_FEE_BASIS_POINTS.*=.*[0-9]" program/src/ > /dev/null 2>&1; then
...
if grep -r "MAX_EMERGENCY_MULTIPLIER.*=.*[0-9]" program/src/ > /dev/null 2>&1; then
...
if grep -r "EconomicState\|total_fees_collected\|total_volume" program/src/ > /dev/null 2>&1; then
...
if grep -A 5 -B 5 "multiplier.*>=.*1.*&&.*multiplier.*<=.*10" program/src/ > /dev/null 2>&1; then
    echo "✅ Found proper multiplier bounds checking"
else
    echo "⚠️  Multiplier bounds checking may be implicit"
fi

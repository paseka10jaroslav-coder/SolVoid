...
if grep -r "check_nullifier_exists\|add_nullifier_to_set\|NullifierAlreadyUsed" program/src/ > /dev/null 2>&1; then
...
if grep -r "nullifier.*validation\|validate_nullifier" program/src/ > /dev/null 2>&1; then
    echo "✅ Found nullifier validation logic"
else
    echo "⚠️  No explicit nullifier validation found"
fi

# Test 3: Check for nullifier set management
echo "Test 3: Checking for nullifier set management"
if grep -r "NullifierSet\|NULLIFIER_SET_SIZE\|nullifiers.*vector" program/src/ > /dev/null 2>&1; then
...
if grep -r "NULLIFIER_PRUNE_DAYS\|prune.*nullifier\|timestamp.*prune" program/src/ > /dev/null 2>&1; then
    echo "✅ Found timestamp-based nullifier pruning"
else
    echo "⚠️  No timestamp-based pruning found"
fi

# Test 5: Check for bitmap optimization
echo "Test 5: Checking for bitmap optimization"
if grep -r "bitmap\|NULLIFIER_BITMAP_SIZE\|bitmap_index" program/src/ > /dev/null 2>&1; then
    echo "✅ Found bitmap optimization for fast lookups"
else
    echo "⚠️  No bitmap optimization found"
fi

# Test 6: Check for merkle root updates
echo "Test 6: Checking for merkle root updates"
if grep -r "merkle_root\|compute_nullifier_merkle_root" program/src/ > /dev/null 2>&1; then
    echo "✅ Found merkle root management"
else
    echo "⚠️  No merkle root management found"
fi

# Test 7: Check for atomic operations
echo "Test 7: Checking for atomic nullifier operations"
if grep -r "add_nullifier_to_set.*atomic\|check.*exists.*before.*add" program/src/ > /dev/null 2>&1; then
    echo "✅ Found atomic nullifier operations"
else
    echo "⚠️  No explicit atomic operations found (may be implicit)"
fi
...
if grep -A 5 -B 5 "check_nullifier_exists.*add_nullifier_to_set" program/src/nullifier_set.rs > /dev/null 2>&1; then
    echo "✅ Found proper nullifier uniqueness check"
else
    echo "⚠️  Nullifier uniqueness check may be implicit"
fi

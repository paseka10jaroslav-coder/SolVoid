...
if grep -r "ark-crypto-primitives\|sha2\|ed25519\|blake2" program/Cargo.toml > /dev/null 2>&1; then
...
if grep -r "version.*=.*\"0\." program/Cargo.toml > /dev/null 2>&1; then
...
if grep -r "ark-\|curve25519\|digest\|signature" program/Cargo.toml > /dev/null 2>&1; then
...
if grep -r "features.*=.*\[.*security.*\]\|features.*=.*\[.*crypto.*\]" program/Cargo.toml > /dev/null 2>&1; then
    echo "✅ Found security features in dependencies"
else
    echo "⚠️  No explicit security features found"
fi

# Test 6: Check for known secure dependency versions
echo "Test 6: Checking for known secure dependency versions"
if grep -r "ark-crypto-primitives.*0\." program/Cargo.toml > /dev/null 2>&1; then
...
if grep -r "dev-dependencies\|build-dependencies" program/Cargo.toml > /dev/null 2>&1; then
    echo "✅ Found proper dependency sections"
else
    echo "⚠️  Limited dependency sections found"
fi

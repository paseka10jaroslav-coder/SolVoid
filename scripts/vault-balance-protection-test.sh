...
if grep -r "vault_balance\|vault.*balance\|get_lamports" program/src/ > /dev/null 2>&1; then
...
if grep -r "InsufficientVaultBalance\|vault_balance.*>=.*required" program/src/ > /dev/null 2>&1; then
...
if grep -r "CIRCUIT_BREAKER_THRESHOLD\|CircuitBreakerTriggered\|pause_withdrawals" program/src/ > /dev/null 2>&1; then
...
if grep -r "MINIMUM_VAULT_RESERVE\|InsufficientVaultReserve\|minimum.*reserve" program/src/ > /dev/null 2>&1; then
...
if grep -r "checked_add\|checked_sub\|atomic.*balance" program/src/ > /dev/null 2>&1; then
...
if grep -r "PRE-VALIDATION\|pre.*validation\|before.*state.*changes" program/src/ > /dev/null 2>&1; then
    echo "✅ Found pre-validation logic"
else
    echo "⚠️  No explicit pre-validation comments found"
fi

# Test 7: Check for withdrawal amount validation
echo "Test 7: Checking for withdrawal amount validation"
if grep -r "withdrawal.*amount\|total_amount.*validation\|deposit_amount" program/src/ > /dev/null 2>&1; then
...
if grep -r "emit!.*CircuitBreakerTriggered\|event.*CircuitBreaker" program/src/ > /dev/null 2>&1; then
...
if grep -r "MINIMUM_VAULT_RESERVE.*=.*[0-9]" program/src/ > /dev/null 2>&1; then
...
if grep -r "CIRCUIT_BREAKER_THRESHOLD.*=.*[0-9]" program/src/ > /dev/null 2>&1; then
...
if grep -A 10 -B 5 "COMPREHENSIVE BALANCE VALIDATION" program/src/lib.rs > /dev/null 2>&1; then
    echo "✅ Found comprehensive balance validation section"
else
    echo "⚠️  No explicit comprehensive validation section found"
fi

...
// Optimized zero hash access - use heap allocation to reduce stack usage
pub fn get_zero_hash(level: usize) -> [u8; 32] {
    // Simple deterministic zero hash computation to avoid large static arrays
    let mut hash = [0u8; 32];
    hash[0] = level as u8;
    hash
}
...
pub mod solvoid_zk {
    ...
    pub fn initialize(ctx: Context<Initialize>, authority: Pubkey) -> Result<()> {
        ...
        // setup subtrees with optimized zero hash computation
        for i in 0..MERKLE_DEPTH {
            state.filled_subtrees[i] = get_zero_hash(i);
        }
        state.merkle_root = get_zero_hash(MERKLE_DEPTH - 1); // the actual root of an empty tree
    ...
    pub fn deposit(
        ...
        // use optimized two-byte hash to reduce stack usage
        for i in 0..MERKLE_DEPTH {
            if (index >> i) & 1 == 0 {
    ...
    // Helper function to validate withdraw parameters (reduces stack usage)
    fn validate_withdraw_params(
        root: &[u8; 32],
    ...
    // Helper function to validate root history (reduces stack usage)
    fn validate_root_history(
        root: &[u8; 32],
        root_history: &Account<RootHistory>,
    ) -> Result<()> {
    ...
    // Helper function to handle transfers (reduces stack usage)
    fn handle_withdrawal_transfers(
    ...
    pub fn withdraw(
        ...
        validate_withdraw_params(&root, &nullifier_hash, amount)?;
        ...
        // Validate root history
        validate_root_history(&root, &ctx.accounts.root_history)?;
        ...
        // Handle economics and transfers
        ...
        handle_withdrawal_transfers(&mut ctx, amount, protocol_fee, fee)?;
...
pub struct ProgramState {
    ...
    pub leaf_count: u64,
    pub filled_subtrees: [[u8; 32]; 16],
    pub is_initialized: bool,
}

impl ProgramState {
    pub const INIT_SPACE: usize = 32 + 32 + 8 + (32 * 16) + 1;
}
...
pub enum PrivacyError {
    ...
    InvalidTimestamp,
    #[msg("Multiplier jump too large")]
    MultiplierJumpTooLarge,
    #[msg("Withdrawal exceeds deposits")]
    WithdrawalExceedsDeposits,
    #[msg("Invalid delay")]
    InvalidDelay,
    #[msg("Multiplier change too frequent")]
    MultiplierChangeTooFrequent,
    #[msg("Thresholds out of sync")]
    ThresholdsOutOfSync,
